...
export const Studio: React.FC = () => {
    ...
    // Scroll progress through pinned section
    const { scrollYProgress } = useScroll({
        target: sectionRef,
        offset: ["start start", "end end"],
    });

    // Smooth the raw scroll signal — raw wheel deltas land in jumps and make
    // the face judder as opacity/scale snap between frames
    ...
    // 1. Fade background to solid dark black matching the next section
    const fadeToDark = useTransform(progress, [0.45, 0.88], [0, 1]);

    // 2. Slowly vanish the face image as the user scrolls
    const faceScrollOpacity = useTransform(progress, [0.42, 0.82], [1, 0]);
    const faceScrollScale = useTransform(progress, [0.42, 0.82], [1, 0.86]);
    const faceScrollY = useTransform(progress, [0.42, 0.82], [0, -45]);
    ...
    return (
        <section
            ref={sectionRef}
            className="relative h-[250vh] w-full select-none"
        >
            {/* STICKY VIEWPORT CONTAINER: Locks the stage in place while scrolling */}
            <div
                onPointerMove={handlePointerMove}
                onPointerLeave={handlePointerLeave}
                className="sticky top-0 h-screen w-full flex items-center justify-center overflow-hidden bg-gradient-to-t from-[#111111]/20 via-[#0a0a0a] to-[#010000]"
            >
                ...
                {/* FACE — centered, uncropped, 3D tilt + vanishes slowly as you scroll */}
                        ...
                        >
                            ...
                            >
                                <motion.div
                                    ...
                                    className="relative w-[42vw] max-w-[720px] aspect-[3/4] will-change-transform [backface-visibility:hidden]"
                                >
                                    <Image
                                        src="/face.png"
                                        alt=""
                                        ...
                                        className="object-contain drop-shadow-[0_40px_80px_rgba(0,0,0,0.85)] transform-gpu [backface-visibility:hidden]"
                                    />
                                    ...
                                    <motion.div
                                        ...
                                        className="absolute inset-0 mix-blend-screen isolate [transform:translateZ(0)] will-change-[background]"
                                    />
                                </motion.div>
                            </motion.div>
                        </motion.div>
